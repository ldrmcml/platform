...
public class DatarecordController implements Controller {
	...
	private void addProduct(HttpServletRequest request,HttpServletResponse response) throws ParseException
	{
		...
		rawmaterial.setName(rm_name);rawmaterial.setManufacturer(rm_manufacturer);rawmaterial.setProductionPlace(rm_place);rawmaterial.setProductionDate(rm_date);
		rawmaterialService.addARawmaterial(rawmaterial);
		int rawmaterialId=rawmaterialService.findRawmaterialId();
